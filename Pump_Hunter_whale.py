...
async def send_early_signal(bot, symbol, data):
    try:
        raw_symbol = symbol.replace("-", "")  # BILLUSDT (для копирования)
        clean_symbol = symbol.replace("-", "").replace("USDT", "/USDT") # BILL/USDT (для чтения)
        ...
        bingx_url = f"https://bingx.com/ru-ru/futures/forward/{raw_symbol}/"
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"📈 График {raw_symbol} на BingX", url=bingx_url)]
        ])
