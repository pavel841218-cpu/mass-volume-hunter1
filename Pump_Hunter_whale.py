    ...
    try:
        async with semaphore:
            ...
            if not context:
                ...
                if context:
                    # 💎 УВЕДОМЛЕНИЕ В TELEGRAM ТОЛЬКО ДЛЯ ТОП-НАКОПЛЕНИЙ (Без спама) 💎
                    if symbol not in consolidation_watchlist:
                        if context['range_pct'] <= 2.2 and context['avg_volume'] >= 8000:
                            clean_symbol = symbol.replace("-", "").replace("USDT", "/USDT")
                            watch_msg = (
                                f"👀 **ИДЕАЛЬНОЕ НАКОПЛЕНИЕ!**\n"
                                f"📊 **{clean_symbol}** | BingX\n\n"
                                f"• Узкий флэт: **{context['range_pct']}%** 🔥\n"
                                f"• Границы: **${format_price(context['range_low'])} - ${format_price(context['range_high'])}**\n"
                                f"• Объём в час: **${int(context['avg_volume']):,}**\n\n"
                                f"⏳ *Бот ждёт 5M-импульс для входа...*"
                            )
                            try:
                                await bot.send_message(chat_id=CHAT_ID, text=watch_msg, parse_mode="Markdown")
                            except Exception as e:
                                logging.error(f"Ошибка отправки вотчлиста {symbol}: {e}")
