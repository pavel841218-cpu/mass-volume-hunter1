...
MAX_ACCUMULATION_VOLUME_SPIKE = 2.5  # Объем в накоплении не должен превышать средний в x2.5
...
MAX_DISTRIBUTION_VOLUME_SPIKE = 2.5  # Объем в дистрибуции не должен превышать средний в x2.5
...
# ===== ВЕБ-СЕРВЕР ДЛЯ РЕНДЕРА (ДЛЯ ПРОХОЖДЕНИЯ PORT SCAN) =====
async def health_check(request):
    return web.Response(text="BingX Scanner Bot is Live!")

async def start_dummy_server():
    """Запуск легкого серверного порта для Render Web Service"""
    app = web.Application()
    ...
    await site.start()
    logging.info(f"🌐 Сервер-заглушка для Render успешно запущен на порту {port}")
        ...
        try:
            await bot.send_message(
                ...
                text="🔄 **Сканер BingX 1H запущен**\n\n"
                     ...
                     f"📊 Накопление: {ACCUMULATION_CANDLES} часов\n"
...
async def main():
    """Точка входа: запускает веб-сервер и сканер одновременно"""
