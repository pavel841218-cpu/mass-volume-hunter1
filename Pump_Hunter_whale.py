...
async def run_history_backtest(bot, test_symbols=["BTC-USDT", "ETH-USDT", "SOL-USDT", "DOGE-USDT", "UB-USDT"]):
    ...
    try:
        await bot.send_message(chat_id=CHAT_ID, text="⏳ **Запуск бэктеста... Анализ истории за 40 часов...**", parse_mode="Markdown")
    except Exception as e:
    ...
    async with aiohttp.ClientSession() as session:
        for symbol in test_symbols:
            ...
            if symbol_signals:
                ...
                report_lines.extend(symbol_signals[-3:])  # Берем последние 3 сигнала для компактности
                report_lines.append("")
            else:
    ...
    try:
        ...
        logging.error(f"Ошибка отправки итогового отчета: {e}")
...
async def main():
    ...
    await run_history_backtest(bot)

    # 2. Переход в режим мониторинга реального времени
    logging.info("🟢 Переход в режим отслеживания онлайн-рынка...")
    while True:
        try:
            # Здесь исполняется основной рабочий цикл сканера
