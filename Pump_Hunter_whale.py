...
# Лимиты объемов
MIN_USDT_VOLUME_5M = 25000
MIN_USDT_VOLUME_1H = 80000
...
def check_accumulation_context(klines_1h):
    ...
    try:
        ...
        # Проверка от пустых списков после фильтрации
...
def detect_early_momentum(klines_5m, context):
    ...
    try:
        ...
        # Проверка volume_mult от деления на ноль
...
async def send_early_signal(bot, symbol, data):
    try:
        ...
        # Расширенное логирование сигнала
        logging.info(
...
async def check_symbol(session, bot, symbol, semaphore):
    """Единый обработчик логики для монет"""
    try:
