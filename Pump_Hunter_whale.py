...
MIN_PRICE = 0.001                     # Минимальная цена
MAX_PRICE = 10.0                      # Максимальная цена
MIN_24H_VOLUME_USDT = 10000.0         # Порог снижен до 10k $, чтобы ловить микро-капы
HIGH_RISK_VOLUME_THRESHOLD = 200000.0 # Все, что ниже 200k $, маркируется как HIGH RISK

# Накопление (8-20 часов)
ACCUMULATION_HOURS = 10
MAX_ACCUMULATION_RANGE_PCT = 12.0     # Расширили флэт до 12% для микро-капов
MAX_ACCUMULATION_VOLUME_SPIKE = 100.0  # Разрешаем сильные выстрелы объема (до х100)

# Пороги Pre-Breakout
PRE_BREAKOUT_DIST_PCT = 1.2           # Расширен допуск близости к уровню
MIN_HARD_RVOL = 1.2                   # Относительный объем (RVOL)
MIN_SCORE_TO_SEND = 40                # Порог прохождения сигнала
...
def analyze_live_setup(klines_5m, context, oi_change_pct=0.0, volume_24h=0.0):
    ...
    try:
        ...
        is_high_risk = volume_24h < HIGH_RISK_VOLUME_THRESHOLD
...
async def send_signal_message(bot, symbol, data):
    try:
        ...
        # Блок предупреждения для неликвида
        warning_header = ""
        if data["is_high_risk"]:
            warning_header = (
                "⚠️ **ВНИМАНИЕ: НИЗКАЯ ЛИКВИДНОСТЬ (HIGH RISK)**\n"
                f"• Суточный объем всего: **${data['volume_24h']:,.0f}**\n"
                "• *Тонкий стакан! Возможны проскальзывания. Входить аккуратно (лимиткой) и сниженным маржированием!*\n\n"
            )
...
async def check_symbol(session, bot, symbol_data, semaphore):
    symbol, volume_24h = symbol_data
    try:
        async with semaphore:
            ...
            result = analyze_live_setup(klines_5m, context, oi_change_pct, volume_24h)
            if result:
...
async def main():
    ...
    logging.info("🚀 Запущен Pre-Breakout & Live Scanner Engine (With High-Risk Detector)")
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=aiohttp.ClientTimeout(total=30)) as session:
        while True:
            try:
                cleanup_storage()
                symbols_data = await fetch_bingx_symbols(session)
                if not symbols_data:
                    await asyncio.sleep(10)
                    continue
                
                tasks = [check_symbol(session, bot, sym_data, semaphore) for sym_data in symbols_data]
