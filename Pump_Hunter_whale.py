...
MAX_ACCUMULATION_RANGE_PCT = 5.0      # Расширено до 5.0%
MAX_ACCUMULATION_VOLUME_SPIKE = 2.5   # Смягчили допуск

# Детектор (5M)
MIN_FIRST_CANDLE_CHANGE_PCT = 0.6    # Снизили до 0.6% для плавного старта
MIN_MOMENTUM_VOLUME_MULT = 2.2       # Смягчили до x2.2 (для лонга и шорта)
MIN_MOMENTUM_CONSECUTIVE = 1         # Ловим с 1-й сильной свечи
MAX_ENTRY_DELAY_CANDLES = 3

# Лимиты объемов
MIN_USDT_VOLUME_5M = 15000           # Порог для низколиквидных монет
MIN_USDT_VOLUME_1H = 50000
...
# ===== ВЕБ-СЕРВЕР ДЛЯ РЕНДЕРА (чтобы не требовал платный тариф) =====
async def handle(request):
    return web.Response(text="Pump Hunter Bot is alive!")
...
async def start_dummy_server():
    ...
    await site.start()
    logging.info(f"🌐 Порт {port} успешно открыт для Render!")
