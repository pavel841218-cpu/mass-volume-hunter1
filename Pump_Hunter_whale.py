...
MAX_COIN_PRICE = 1.0  # Искать монеты ТОЛЬКО дешевлe 1.0 USDT
...
# Лимиты объемов (Настроено под чистые пробои дешевых монет)
    ...
    try:
        async with semaphore:
            ...
            if not context:
                ...
                # ===== ФИЛЬТР ЦЕНЫ (ТОЛЬКО МОНЕТЫ ДЕШЕВЛЕ 1 USDT) =====
                try:
...
async def main():
    ...
    logging.info("🚀 Сканер BingX запущен (только монеты < 1.0 USDT)")
