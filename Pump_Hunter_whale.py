...
THRESHOLD_VOL = 1.3          # Порог всплеска объема (раз)
...
class AutoVolumeMonitor:
    ...
    async def update_market_pairs(self):
        global WATCH_PAIRS
        logger.info("🔄 Поиск монет в диапазоне 0.01$ - 1$ с объемом...")
        try:
    ...
    async def check_pair(self, pair: dict):
        ...
        # Отладочный лог: проверяем наполняемость массивов свечей
        ...
        try:
            ...
            # Выводим текущие коэффициенты в лог
            ...
            # Мягкое условие: сигнал улетит, если хотя бы на одной бирже объем > 1.3
